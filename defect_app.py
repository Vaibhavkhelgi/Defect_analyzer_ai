...
# STREAMLIT PAGE
st.set_page_config(
    page_title='Structural Defect Analysis using AI',
    page_icon='⚠️',
    layout='wide'
)

st.title('AI Assistant for structural defect analysis ⚠️')
st.header(':blue[Prototype for automated structural defect analysis]')
st.subheader(''' Develop a web based app using Streamlit that allows users to upload image of a building structures and to analyze the defects using a Gemini Model''',divider=False)

with st.expander('About the application'):
    st.markdown(f''' This is used to detect the structural defect in given images
                like craks, misallignments using AI system
                - ***Defect Detection***
                - ***Recommendations***
                - ***Report Generation***''')
...
in more precise way(maximum 3 points) for the following questions:
...
8. Say whether we need to monito the defected area after repair or replacement?
9. Give the range to repair the defect or replace in Indian rupees.
...
def generate_results(prompt,img):
    model = genai.GenerativeModel('gemini-2.5-flash') # gemini-1.5-pro , gemini-1.5-flash, gemini-2.0-pro  ,gemini-2.0-flash
...
submit = st.button('Analyze the defects 👨🏻‍💻')
